...
<script lang="ts">
	...
	let name = $derived(skin?.n);
	let feet = $derived(skin?.f);
	let body = $derived(skin?.b);

	let skinURI = $state(X_SPEC_SKIN);
	let loadingSkin = $state() as string | undefined;
	...
	const updateSkin = async () => {
		...
		if (response && response.ok) {
			...
			if (contentType.startsWith('image/')) {
				...
				skinURI = `data:${contentType};base64,${encodeBase64(buffer)}`;
			} else {
				skinURI = body && feet ? DEFAULT_SKIN_GS : DEFAULT_SKIN;
				loadingSkin = 'default';
			}
		} else {
			skinURI = body && feet ? DEFAULT_SKIN_GS : DEFAULT_SKIN;
...
{#if !hide}
	<div
		...
		{...rest}
		style="background-image: url({skinURI}); {pose
